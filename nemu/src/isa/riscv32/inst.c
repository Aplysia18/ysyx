...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, {R(rd) = s->pc + 4; s->dnpc = s->pc + imm; if(rd==1) ftrace_call(s->pc, s->dnpc); btrace_write(jal, s->pc, s->isa.inst.val, s->dnpc);});
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, {R(rd) = s->pc + 4; s->dnpc = src1 + imm; if(rd==1) ftrace_call(s->pc, s->dnpc); else if(s->isa.inst.val==0x00008067) ftrace_ret(s->pc); btrace_write(jalr, s->pc, s->isa.inst.val, s->dnpc);});
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    , B, {if (src1 == src2) s->dnpc = s->pc + imm; btrace_write(branch, s->pc, s->isa.inst.val, s->dnpc);});
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne    , B, {if (src1 != src2) s->dnpc = s->pc + imm; btrace_write(branch, s->pc, s->isa.inst.val, s->dnpc);});
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, {if ((sword_t)src1 < (sword_t)src2) s->dnpc = s->pc + imm; btrace_write(branch, s->pc, s->isa.inst.val, s->dnpc);});
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, {if ((sword_t)src1 >= (sword_t)src2) s->dnpc = s->pc + imm; btrace_write(branch, s->pc, s->isa.inst.val, s->dnpc);});
  INSTPAT("??????? ????? ????? 110 ????? 11000 11", bltu   , B, {if (src1 < src2) s->dnpc = s->pc + imm; btrace_write(branch, s->pc, s->isa.inst.val, s->dnpc);});
  INSTPAT("??????? ????? ????? 111 ????? 11000 11", bgeu   , B, {if (src1 >= src2) s->dnpc = s->pc + imm; btrace_write(branch, s->pc, s->isa.inst.val, s->dnpc);});
