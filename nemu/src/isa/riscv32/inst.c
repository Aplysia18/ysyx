...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, {R(rd) = s->pc + 4; s->dnpc = src1 + imm; if(rd==1) ftrace_call(s->pc, s->dnpc);});
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    , B, {if (src1 == src2) s->dnpc = s->pc + imm; if(rd==1) ftrace_call(s->pc, s->dnpc); else if(s->isa.inst.val==0x00008067) ftrace_ret(s->pc);});
