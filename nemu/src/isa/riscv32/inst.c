...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, {R(rd) = s->pc + 4; s->dnpc = src1 + imm; ftrace_call(s->pc, s->dnpc);});
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    , B, {if (src1 == src2) s->dnpc = s->pc + imm; ftrace_call(s->pc, s->dnpc);});
