...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, {R(rd) = s->pc + 4; s->dnpc = src1 + imm; if(rd==1) ftrace_call(s->pc, s->dnpc); else if(s->isa.inst.val==0x00008067) ftrace_ret(s->pc);});
  ...
  /*----------Zicsr----------*/
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , I, {R(rd) = CSRs(imm); CSRs(imm) = src1; printf("csrrw: csr %d = 0x%08x\n", imm, CSRs(imm));});
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , I, {R(rd) = CSRs(imm); CSRs(imm) |= src1; printf("csrrs: csr %d = 0x%08x\n", imm, CSRs(imm));});
